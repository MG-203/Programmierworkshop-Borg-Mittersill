...
explosionImages = []
for i in range(1, 36):
    img = pygame.image.load(f"explosion_frames/explosion_{i}.png")
    explosionImages.append(img)

explosions = []

def startExplosion(x, y):
    explosions.append({
        "x": x,
        "y": y,
        "frame": 0,
...
def updateExplosions():
    for explosion in explosions[:]:
        ...
        screen.blit(image, (explosion["x"], explosion["y"]))

        explosion["counter"] += 1

        # Je größer diese Zahl, desto langsamer die Animation
        if explosion["counter"] >= 10:
            explosion["counter"] = 0
            explosion["frame"] += 1

            if explosion["frame"] >= len(explosionImages):
...
# Reset Game 
def resetGame():
    global playerX, playerY, changeX, enemyX, enemyY, changeEnemyX, changeEnemyY
...
while running: 
    ...
    for i in range(NumberOfEnemies):
        ...
        if collision: 
            ...
            startExplosion(enemyX[i], enemyY[i])
